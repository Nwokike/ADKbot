...
from adkbot.channels.matrix import _build_matrix_text_content

import adkbot.channels.matrix as matrix_module
from adkbot.bus.events import OutboundMessage
from adkbot.bus.queue import MessageBus
from adkbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from adkbot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("adkbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "adkbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("adkbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "adkbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
...
def test_build_matrix_text_content_no_event_id() -> None:
    ...
    assert "m.relates_to" not in str(result)
    assert "m.new_content" not in str(result)
    assert "format" not in str(result)
    assert "formatted_body" not in str(result)
