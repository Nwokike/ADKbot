"""Context builder for assembling agent prompts with ADK instruction support."""
...
from adkbot.utils.helpers import build_assistant_message, current_time_str, detect_image_mime


class ContextBuilder:
    """Builds context and instructions for ADK agents."""
...
{skills_summary}""")
    ...
    def build_adk_instruction(self, skill_names: list[str] | None = None) -> str:
        """Build instruction string for ADK Agent.instruction parameter.

        This method generates the instruction that can be passed to an ADK LlmAgent.
        It includes identity, bootstrap files, and skills context.
        """
        parts = [self._get_identity()]
...
{skills_summary}""")
    ...
    def build_adk_instruction_with_state(
        self, state: dict[str, Any], skill_names: list[str] | None = None
    ) -> str:
        """Build instruction with ADK state variables substituted.

        ADK supports template variables like {user_name}, {preferences}, etc.
        ...
        Returns:
            Instruction string with state variables substituted
        """
        instruction = self.build_adk_instruction(skill_names)
...
Example: send_message(content="Here is the file", media=["/path/to/file.png"])"""
    ...
    def build_messages(
        ...
        Note: This method is kept for backward compatibility. ADK's Runner
        handles message construction internally when using LlmAgent.
        """
    ...
    def get_memory_context(self) -> str:
        """Get memory context for external use (e.g., ADK tools)."""
        return self.memory.get_memory_context()

    async def search_memory(
        self, query: str, app_name: str = "adkbot", user_id: str = "default"
    ) -> list[dict[str, Any]]:
        """Search memory using ADK MemoryService if available."""
        return await self.memory.search_adk_memory(app_name, user_id, query)