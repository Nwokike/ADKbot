...
from adkbot.config.schema import ChannelsConfig
...
def test_discover_plugins_loads_entry_points():
    from adkbot.channels.registry import discover_plugins
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("adkbot.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "adkbot.channels.registry.discover_all",
...
def test_channels_login_sets_custom_config_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("adkbot.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "adkbot.config.loader.set_config_path",
...
def test_channels_status_sets_custom_config_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("adkbot.channels.registry.discover_all", lambda: {})
...
def test_builtin_channel_default_config():
    ...
    from adkbot.channels.telegram import TelegramChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("adkbot.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("adkbot.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("adkbot.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
