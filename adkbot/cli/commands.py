...
from adkbot.cli.stream import StreamRenderer, ThinkingSpinner
from adkbot.config.paths import get_workspace_path, is_default_workspace
from adkbot.config.schema import Config
from adkbot.utils.helpers import sync_workspace_templates


try:
    from adkbot.agent import AdkAgentLoop, create_adk_loop
...
def _init_prompt_session() -> None:
    ...
    from adkbot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    c.print()
    c.print(f"[cyan]{__logo__} adkbot[/cyan]")
    c.print(body)
    c.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} adkbot v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    # Run interactive wizard unless skipped
    if not skip_wizard:
        from adkbot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'adkbot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
...
def _get_adk_config(config: Config) -> dict[str, Any]:
    """Extract ADK-compatible configuration from Config object."""
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from adkbot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show adkbot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'adkbot[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from adkbot.api.server import create_app
    from adkbot.bus.queue import MessageBus

    if verbose:
        logger.enable("adkbot")
    else:
        logger.disable("adkbot")
...
def gateway(
    ...
    from adkbot.cron.types import CronJob
    from adkbot.heartbeat.service import HeartbeatService
    ...
    console.print(f"{__logo__} Starting adkbot gateway version {__version__} on port {port}...")
    sync_workspace_templates(runtime_config.workspace_path)
    bus = MessageBus()
    ...
    # Persistent logging
    log_dir = runtime_config.workspace_path.parent / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    logger.add(log_dir / "adkbot.log", rotation="10 MB", level="DEBUG" if verbose else "INFO")

    logger.info("Initializing agent loop")
    agent = create_adk_loop(
        bus=bus,
        workspace=runtime_config.workspace_path,
        config=runtime_config,
        timezone=runtime_config.agents.defaults.timezone,
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to and response:
            should_notify = await evaluate_response(
                response,
                job.payload.message,
                model=runtime_config.get_effective_model(),
            )
            if should_notify:
...
def agent(
    ...
    logs: bool = typer.Option(
        False, "--logs/--no-logs", help="Show adkbot runtime logs during chat"
    ),
...
def _get_bridge_dir() -> Path:
    ...
    from adkbot.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # adkbot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall adkbot")
        raise typer.Exit(1)
...
def status():
    ...
    console.print(f"{__logo__} adkbot Status\n")
...
def install_service(
    ...
    if sys.platform == "win32":
        console.print("[red]Linux systemd is required for this command.[/red]")
        raise typer.Exit(1)
    ...
    cli_path = shutil.which("adkbot")
    if not cli_path:
        console.print(
            "[red]Could not locate 'adkbot' in PATH. Make sure it is installed globally (e.g. uv tool install .)[/red]"
        )
        raise typer.Exit(1)

    service_content = f"""[Unit]
Description=ADKBot Gateway Service
...
"""
    ...
    console.print(f"[green]✓[/green] Created service file at {service_file}")

    try:
        subprocess.run(["systemctl", "--user", "daemon-reload"], check=True)
        subprocess.run(["systemctl", "--user", "enable", "--now", service_name], check=True)
        console.print(
            f"[green]✓[/green] Systemd service '{service_name}' enabled and started successfully!"
        )
        console.print(
            f"  [dim]Type 'adkbot logs' or 'journalctl --user -u {service_name} -f' to view output.[/dim]"
        )
    except Exception as e:
